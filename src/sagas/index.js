...
function* authorize() {
    ...
    try {
        ...
        if (!user) {
            ...
            switch (request.type) {
                case GET_EMAIL:
                    user = yield cps(authentication.loginEmail, request.payload);
                    break;
                case GET_FACEBOOK:
...
function* logout() {
    ...
    try {
        const response = yield call(authentication.logout);
        yield put({ type: "SENDING_REQUEST", sending: false });
...
// //     yield takeLatest([GET_GOOGLE_SUCCESS, GET_FACEBOOK_SUCCESS, GET_EMAIL_SUCCESS], navigateTo);
// // }
...
// export default function* rootSaga() {
//     yield fork(loginFlow), yield fork(registerFlow), yield fork(logoutFlow);
//     // yield all([
//     //     watchLogout(),
//     // ]);
// }
